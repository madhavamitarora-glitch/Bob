...
A personal campus assistant for Purdue West Lafayette, in two halves that
...
for push notifications (no account), and SQLite (no hosted database). No
LLM API calls happen anywhere in the running system — matching is plain
Python string/regex scoring.
...
9. **Add the MCP server to Claude Desktop** — see the config block below.
...
## Claude Desktop config (the MCP server)
...
Adjust the `command` path if you use a different Python install (run
`python -c "import sys; print(sys.executable)"` to find yours), and the
`args` path if you clone the repo somewhere else. Restart Claude Desktop
...
```
bob/
  poll.py                      # fetch, score, store, notify
